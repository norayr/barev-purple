 ...
 * Adapted from libpurple/protocols/jabber/jingle/transport.c
...
#define JINGLE_TRANSPORT_GET_PRIVATE(obj) \
	(G_TYPE_INSTANCE_GET_PRIVATE((obj), JINGLE_TYPE_TRANSPORT, JingleTransportPrivate))
...
jingle_transport_get_type()
{
	...
	if (type == 0) {
		static const GTypeInfo info = {
			sizeof(JingleTransportClass), NULL, NULL,
			(GClassInitFunc) jingle_transport_class_init,
			NULL, NULL, sizeof(JingleTransport), 0,
			(GInstanceInitFunc) jingle_transport_init, NULL
		};
...
static void
jingle_transport_set_property(GObject *object, guint prop_id,
...
static void
jingle_transport_get_property(GObject *object, guint prop_id,
...
xmlnode *
jingle_transport_to_xml(JingleTransport *transport, xmlnode *content,
