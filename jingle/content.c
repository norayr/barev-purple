 ...
 * Adapted from libpurple/protocols/jabber/jingle/content.c
 * No JabberStream references — minor include changes only.
 */
...
#define JINGLE_CONTENT_GET_PRIVATE(obj) \
	(G_TYPE_INSTANCE_GET_PRIVATE((obj), JINGLE_TYPE_CONTENT, JingleContentPrivate))
...
jingle_content_get_type()
{
	...
	if (type == 0) {
		static const GTypeInfo info = {
			sizeof(JingleContentClass), NULL, NULL,
			(GClassInitFunc) jingle_content_class_init,
			NULL, NULL, sizeof(JingleContent), 0,
			(GInstanceInitFunc) jingle_content_init, NULL
		};
...
jingle_content_class_init(JingleContentClass *klass)
{
	...
	g_object_class_install_property(gobject_class, PROP_SESSION,
			g_param_spec_object("session", "Jingle Session",
			"The jingle session parent of this content.",
			JINGLE_TYPE_SESSION, G_PARAM_READWRITE));
	g_object_class_install_property(gobject_class, PROP_CREATOR,
			g_param_spec_string("creator", "Creator",
			"The participant that created this content.", NULL,
			G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
	g_object_class_install_property(gobject_class, PROP_DISPOSITION,
			g_param_spec_string("disposition", "Disposition",
			"The disposition of the content.", NULL,
			G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
	g_object_class_install_property(gobject_class, PROP_NAME,
			g_param_spec_string("name", "Name",
			"The name of this content.", NULL,
			G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
	g_object_class_install_property(gobject_class, PROP_SENDERS,
			g_param_spec_string("senders", "Senders",
			"The sender of this content.", NULL,
	...
	g_object_class_install_property(gobject_class, PROP_PENDING_TRANSPORT,
			g_param_spec_object("pending-transport", "Pending transport",
			"The pending transport contained within this content",
			JINGLE_TYPE_TRANSPORT, G_PARAM_READWRITE));
...
static void
jingle_content_set_property(GObject *object, guint prop_id,
...
static void
jingle_content_get_property(GObject *object, guint prop_id,
...
JingleSession *
jingle_content_get_session(JingleContent *content)
{
	JingleSession *session;
...
void
jingle_content_set_pending_transport(JingleContent *content,
		JingleTransport *transport)
{
...
{
	const gchar *type =
			xmlnode_get_namespace(xmlnode_get_child(content, "description"));
...
static xmlnode *
jingle_content_to_xml_internal(JingleContent *content, xmlnode *jingle,
		JingleActionType action)
{
	...
	if (action != JINGLE_CONTENT_REMOVE) {
		...
		if (action != JINGLE_TRANSPORT_REJECT &&
				action == JINGLE_TRANSPORT_REPLACE)
...
xmlnode *
jingle_content_to_xml(JingleContent *content, xmlnode *jingle,
		JingleActionType action)
{
...
void
jingle_content_handle_action(JingleContent *content, xmlnode *xmlcontent,
		JingleActionType action)
{
