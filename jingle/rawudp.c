 ...
 * Adapted from libpurple/protocols/jabber/jingle/rawudp.c
 * No JabberStream dependency — minor include changes only.
 */
...
#define JINGLE_RAWUDP_GET_PRIVATE(obj) \
	(G_TYPE_INSTANCE_GET_PRIVATE((obj), JINGLE_TYPE_RAWUDP, JingleRawUdpPrivate))
...
{
	JingleRawUdpCandidate *c = g_new0(JingleRawUdpCandidate, 1);
	c->generation = candidate->generation;
	c->component = candidate->component;
	c->id = g_strdup(candidate->id);
	c->ip = g_strdup(candidate->ip);
	c->port = candidate->port;
	c->rem_known = candidate->rem_known;
	return c;
}
...
jingle_rawudp_get_type()
{
	...
	if (type == 0) {
		static const GTypeInfo info = {
			sizeof(JingleRawUdpClass), NULL, NULL,
			(GClassInitFunc) jingle_rawudp_class_init,
			NULL, NULL, sizeof(JingleRawUdp), 0,
			(GInstanceInitFunc) jingle_rawudp_init, NULL
		};
...
jingle_rawudp_class_init(JingleRawUdpClass *klass)
{
	...
	g_object_class_install_property(gobject_class, PROP_LOCAL_CANDIDATES,
			g_param_spec_pointer("local-candidates", "Local candidates",
			"The local candidates for this transport.", G_PARAM_READABLE));
	g_object_class_install_property(gobject_class, PROP_REMOTE_CANDIDATES,
			g_param_spec_pointer("remote-candidates", "Remote candidates",
			"The remote candidates for this transport.", G_PARAM_READABLE));
...
static void
jingle_rawudp_set_property(GObject *object, guint prop_id,
		const GValue *value, GParamSpec *pspec)
{
	...
	switch (prop_id) {
		case PROP_LOCAL_CANDIDATES:
			rawudp->priv->local_candidates = g_value_get_pointer(value);
			break;
		case PROP_REMOTE_CANDIDATES:
			rawudp->priv->remote_candidates = g_value_get_pointer(value);
			break;
		default:
...
static void
jingle_rawudp_get_property(GObject *object, guint prop_id,
...
void
jingle_rawudp_add_local_candidate(JingleRawUdp *rawudp,
	...
	for (; iter; iter = g_list_next(iter)) {
		...
		if (purple_strequal(c->id, candidate->id)) {
			...
			rawudp->priv->local_candidates =
					g_list_delete_link(rawudp->priv->local_candidates, iter);
			candidate->generation = generation;
			rawudp->priv->local_candidates =
					g_list_append(rawudp->priv->local_candidates, candidate);
			return;
		}
	}
	rawudp->priv->local_candidates =
...
static void
jingle_rawudp_add_remote_candidate(JingleRawUdp *rawudp,
		JingleRawUdpCandidate *candidate)
{
...
jingle_rawudp_parse_internal(xmlnode *rawudp)
{
	...
	for (; candidate; candidate = xmlnode_get_next_twin(candidate)) {
		...
		rawudp_candidate = jingle_rawudp_candidate_new(
				id, atoi(generation), atoi(component), ip, atoi(port));
...
static xmlnode *
jingle_rawudp_to_xml_internal(JingleTransport *transport, xmlnode *content,
